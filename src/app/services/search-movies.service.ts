...
export class SearchService {
    ...
    public getMoviesInformations(searchTerm: string): Observable<any> {
        ...
        return this.http.get(`${this.url}/?s=${searchTerm}&apikey=${this.apiKey}`).pipe(catchError((error: HttpErrorResponse) => this.formatResponseErrors(error)));
    }

    private formatResponseErrors(error: HttpErrorResponse) {
        return throwError({ error: { error: error.error, status: error.status, statusText: error.statusText }});
    }
}
